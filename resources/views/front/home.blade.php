...
========================= --}}
    ...
    ->with('company') // ロゴ取得のため必須
    ->orderByDesc('id')
    ->limit(8)
    ->get()
    ->map(function($j) use ($imgCols, $normalize) {
    // 1) 求人画像
    $img = null;
    foreach ($imgCols as $col) {
    if (!empty($j->{$col})) { $img = $j->{$col}; break; }
    }
    $img = $normalize($img);

    // 2) 会社ロゴ（accessorがあれば logo_url を、無ければ列から解決）
    $logo = null;
    if ($j->company) {
    $c = $j->company;
    $logo = $c->logo_url ?? null;
    if (!$logo) {
    foreach (['logo_path','logo','thumbnail_path'] as $col) {
    if (!empty($c->{$col})) { $logo = $normalize($c->{$col}); if ($logo) break; }
    }
    }
    }
    ...
    <section class="mt-12">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <a href="{{ $jobUrl }}" class="group rounded-2xl bg-white border shadow-sm hover:shadow-md transition overflow-hidden">
          <div class="aspect-[16/9] bg-gray-100 overflow-hidden flex items-center justify-center">
            @if (!empty($job['image']))
            <img src="{{ $job['image'] }}"
              class="w-full h-full object-cover group-hover:scale-[1.01] transition" alt="">
