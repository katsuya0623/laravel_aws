        ...
        <form method="POST" action="{{ route('front.jobs.apply_store', ['job' => $job->slug ?? $job->id]) }}">
            ...
            <div class="fm-row">
                ...
                <div class="readonly row-gender">
                    {!! $displayOrUnset($p->gender_label ?? $u->gender_label ?? null) !!}
                    <input type="hidden" name="gender" value="{{ $p->gender ?? $u->gender ?? '' }}">
                </div>
            </div>
            ...
            <div class="fm-row">
                ...
                <div class="readonly row-birth">
                    @php
                    $birthRaw = $p->birthdate ?? $u->birthdate ?? null;
                    $birthDisp = $birthRaw ? \Illuminate\Support\Carbon::parse($birthRaw)->isoFormat('YYYY年M月D日') : null;
                    @endphp
            ...
            <div class="fm-row">
                ...
                <div class="readonly row-addr">
                    {!! $displayOrUnset($p->address_full ?? $u->address_full ?? null) !!}
                    <input type="hidden" name="address" value="{{ $p->address ?? $u->address ?? '' }}">
                </div>
            </div>
            ...
            <div class="fm-row">
                ...
                <div>
                    <input type="text" name="phone" class="fm-input" value="{{ old('phone', $p->phone ?? $u->phone ?? '') }}" placeholder="090-1234-5678">
            ...
            <div class="fm-row">
                ...
                <div class="readonly row-edu">
                    {!! $displayOrUnset($p->education_summary ?? $u->education_summary ?? null) !!}
                    <input type="hidden" name="education" value="{{ $p->education ?? $u->education ?? '' }}">
                </div>
            </div>
            ...
            {{-- 希望する雇用形態（選択） → desired_type（Controllerでalias吸収） --}}
            <div class="fm-row">
            ...
            {{-- 自己PR / 自由記述欄（＝ controller の message フィールド） --}}
            <div class="fm-row">
...
{{-- 追加項目（カナ/各セレクト/志望動機/表示項目）を message の末尾に併記して送る --}}
<script>
    (function() {
