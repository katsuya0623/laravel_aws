...
/* ===== ログイン導線 ===== */
Route::get('/company/login', fn(Request $r) => redirect()->route('login.intended', ['redirect' => route('dashboard')]))
    ->name('company.login');

Route::get('/mypage/login', fn(Request $r) => redirect()->route('login.intended', ['redirect' => route('dashboard')]))
    ->name('mypage.login');
...
// ★ throttle:admin-login を追加（管理者ログイン失敗制限）
if (! \Illuminate\Support\Facades\Route::has('admin.login')) {
    Route::prefix('admin')->name('admin.')
        ->middleware(['web', 'guest:admin', 'throttle:admin-login'])
        ->group(function () {
...
Route::prefix('admin')->middleware(['auth:admin'])->name('admin.')->group(function () {
    ...
    Route::get('__alias/filament-posts/{post}/edit', fn($post) => redirect(PostResource::getUrl('edit', ['record' => $post], panel: 'admin')))
        ->whereNumber('post')
        ->name('posts.edit');
    ...
    Route::get('recruit_jobs', fn() =>
        redirect()->to(
            Route::has("filament.admin.resources.$recruitSlug.index")
                ? RecruitJobResource::getUrl('index')
                : url('/admin/recruit_jobs')
        )
    )->name('jobs.index');

    Route::get('recruit_jobs/create', fn() =>
        redirect()->to(
            Route::has("filament.admin.resources.$recruitSlug.create")
                ? RecruitJobResource::getUrl('create')
                : url('/admin/recruit_jobs')
        )
    )->name('jobs.create');

    Route::get('recruit_jobs/{job}', fn($job) =>
        redirect()->to(
            Route::has("filament.admin.resources.$recruitSlug.edit")
                ? RecruitJobResource::getUrl('edit', ['record' => $job])
                : url("/admin/recruit_jobs/{$job}/edit")
        )
    )->whereNumber('job')->name('jobs.show');

    Route::get('recruit_jobs/{job}/edit', fn($job) =>
        redirect()->to(
            Route::has("filament.admin.resources.$recruitSlug.edit")
                ? RecruitJobResource::getUrl('edit', ['record' => $job])
                : url("/admin/recruit_jobs/{$job}/edit")
    ...
    Route::get('__alias/filament-applications', fn() => redirect()->route('filament.admin.resources.applications.index'))
